...
class Engine:
    ...
    def __init__(self, preprocess_script_name=None, postprocess_script_name=None):
        self.preprocess_script = self.make_path(
            "preprocess" if preprocess_script_name is None else preprocess_script_name
        )
        self.postprocess_script = self.make_path(
            "postprocess"
            if postprocess_script_name is None
            else postprocess_script_name
        )
        ...
        if not (output_dir / "slidy").exists():
            super().run_cmd(
                [
                    "git",
                    "clone",
                    "https://github.com/slideshow-templates/slideshow-slidy.git",
                    str(output.parent / "slidy"),
                ],
                "fetching slidy data files",
            )
        ...
        if not (output_dir / "reveal.js").exists():
            super().run_cmd(
                [
                    ...
                    str(output.parent / "reveal.js"),
                ],
                "fetching reveal.js data files",
            )
